...
try:
    data_2 = pandas.read_csv("data/words_to_learn")
except FileNotFoundError:
    original_data = pandas.read_csv("data/python_terms.csv")
    terms_to_learn = original_data.to_dict(orient="records")
else:
    terms_to_learn = data_2.to_dict(orient="records")

def next_card():
    global current_card, flip_timer
    window.after_cancel(flip_timer)
    current_card = random.choice(terms_to_learn)
...
def is_known():
    terms_to_learn.remove(current_card)
...
card_word = canvas.create_text(400, 263, text="", font=("Ariel", 12, "bold"))
